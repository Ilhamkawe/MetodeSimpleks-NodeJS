...
describe('SimpleSimplex tableau helpers', () => {
  it('should generate tableau rows properly', () => {
    expect(
      generateTableauRow(generateTableauProblem()),
      'tableau row is not generated properly with row no: 0'
    ).to.deep.equal([3, 5, 7, 11, 13, 1, 0, 0, 17]);

    expect(
      generateTableauRow(generateTableauProblem({
        rowNo: 1,
      })),
      'tableau row is not generated properly with row no: 1'
    ).to.deep.equal([3, 5, 7, 11, 13, 0, 1, 0, 17]);

    expect(
      generateTableauRow(generateTableauProblem({
        rowNo: 2,
      })),
      'tableau row is not generated properly with row no: 2'
    ).to.deep.equal([3, 5, 7, 11, 13, 0, 0, 1, 17]);
  });

  it('should convert constraints to preRow form', () => {
    expect(
      constraintToPreRow(generateConstraint()),
      'tableau row was not correctly converted into pre-row format'
    ).to.deep.equal({
...
describe('Simple simplex pivot helpers', () => {
  it('should return -1 for non-negative last rows', () => {
    ...
    expect(
      ...
      'non negative last row created non -1 solution'
    ).to.equal(-1);
  });
  ...
  it('should find the pivot column index properly', () => {
    ...
    expect(
      getPivotColumnIndex(firstTableau),
      'pivot for first tableau was wrong'
    ).to.equal(0);

    expect(
      getPivotColumnIndex(secondTableau),
      'pivot for second tableau was wrong'
    ).to.equal(1);

    expect(
      getPivotColumnIndex(thirdTableau),
      'pivot for third tableau was wrong'
    ).to.equal(5);
  });
  ...
  it('should find the pivot row index properly', () => {
    ...
    expect(
      getPivotRowIndex(tableau),
      'pivot row index was wrong'
    ).to.equal(2);
  });
  ...
  it('should find the pivot coords properly', () => {
    const tableau = generateTableau();
    expect(
      getPivotCoords(tableau),
      'pivot coords were wrong'
    ).to.deep.equal({
      rowNo: 2,
      colNo: 5,
    });
  ...
  it('should adjust pivot row', () => {
    ...
    expect(
      adjustedPivotRow,
      'pivot row is not adjusted properly'
  ...
  it('should do elementary row operations to pivot rows', () => {
    ...
    expect(
      adjustedInputRow,
      'EROs failed to adjust the non pivot row'
